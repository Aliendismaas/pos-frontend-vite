  ...
  const handleStepSubmit = async (stepData) => {
    ...
    if (step === 1) {
      ...
      try {
        ...
        const errorMessage = err?.message 
          || err?.error 
          || (typeof err === 'string' ? err : 'Store creation failed. Please try again.');
        setLocalError(errorMessage);
        setLocalLoading(false);
      }
    }
  ...
  const handleStepBack = () => {
    if (step > 1) {
      ...
      setTimeout(() => {
        setStep(step - 1);
        setFadeIn(true);
        setLocalError(null); // Clear errors when going back
      }, 150);
    }
  };
          ...
          <div className="w-full max-w-md">
            ...
            {(error || localError) && (
              <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg shadow-sm">
                <div className="flex items-center">
                  ...
                  <span>{getErrorMessage(error || localError)}</span>
                </div>
              </div>
            )}
              ...
              <div className="fixed inset-0 flex items-center justify-center bg-white/60 z-50">
                <div className="text-center">
                  ...
                  <p className="text-gray-600">
                    {step === 1 ? 'Creating your account...' : 'Setting up your store...'}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="mt-8 text-center">
              <p className="text-sm text-gray-500">
                ...
                <Link to="/auth/login" className="text-primary hover:text-green-900 font-medium">
                  Sign in here
                </Link>
              </p>
            </div>
